...
+   如果`A`是对称的，则A的特征值是实数并且 $A =Q \Lambda Q^T$
...
幂 $A^2$ 将为你提供，通过两步从一个页面到另一个页面有多少种方式。 你可以在[这些笔记](http://www.utdallas.edu/~jwz120030/Teaching/PastCoursesUMBC/M221HS06/ProjectFiles/Adjacency.pdf)中看到更详细的示例，如适用于航空旅行。
...
定理：如果`X`是非奇异的，那么`A`和 $X^{-1}AX$ 具有相同的特征值。
...
$Q_k, R_k = A_{k-1} \\ A_k = R_kQ_k$

我们可以将其视为构建 $A_k$，$Q_k$ 和 $R_k$ 的序列。

$A_k = Q_k \, R_k \\ Q_k^{-1} \, A_k = R_k$

所以：

$R_k Q_k = Q_k^{-1} \, A_k \, Q_k \\ A_k = Q_k^{-1} \ldots Q_2^{-1} Q_1^{-1} A Q_1 Q_2 \dots Q_k$

Trefethen 在第 216-217 页证明了以下内容：

$A^k = Q_1 Q_2 \dots Q_k R_k R_{k-1}\dots R_1$

要点：QR 算法为连续的幂构造 $A_k$ 的正交基。 记住`A`的幂与特征值分解之间的密切关系。
...
选择 $s_k$ 来近似`A`的特征值。我们将使用 $s_k = A_k(m,m)$。
...
当向量`b`投影到直线`a`上时，其投影`p`是`b`沿着直线`a`的一部分。 所以`p`是`a`的一些倍数。 设 $\mathbf{p} = \hat{x}\mathbf{a}$ 其中 $\hat{x}$ 是标量。

### 正交性

投影的关键是正交性：从`b`到`p`的直线（可以写成 $\mathbf{b} - \hat{x}\mathbf{a}$）垂直于`a`。
...
其中 $Q_{n + 1}$ 为 $n \times n + 1$，$H_n$ 为 $n + 1 \times n$。 这会创建一个可解决的递归关系。
